  ...
  const sidebarContent = (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 px-5 py-5">
        ...
        <button
          onClick={close}
          className="flex size-7 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted md:hidden"
        >
          <X className="size-4" />
        </button>
      </div>
      ...
      <div className="mx-3 mb-3">
        ...
        >
          ...
          <ChevronDown className={cn("size-4 shrink-0 text-muted-foreground transition-transform duration-200", showWorkspaces && "rotate-180")} />
        </button>
              ...
              {workspaces.map((ws) => (
                <button
                  ...
                  className={cn(
                    ...
                    ws.is_active ? "bg-primary/8 text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"
                  )}
                >
                  <span className="flex size-6 shrink-0 items-center justify-center rounded-md text-[11px] font-bold text-white" style={{ backgroundColor: ws.color }}>
      ...
      {/* Nav label */}
      <p className="px-5 pb-1.5 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">Menu</p>
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 pb-4">
        {NAV_LINKS.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon className={cn("size-4 shrink-0 transition-transform group-hover:scale-110", isActive && "group-hover:scale-100")} />
              {label}
            </Link>
          );
        })}
      ...
      <Dialog open={showLogoutConfirm} onOpenChange={setShowLogoutConfirm}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" onClick={() => setShowLogoutConfirm(false)} disabled={signingOut} className="flex-1">Cancel</Button>
            <Button variant="destructive" onClick={handleLogout} disabled={signingOut} className="flex-1 gap-2">
